...
async function applyNoShow(adminId, appointmentId, reason) {
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (new Date() <= dueAt) {
            throw new BusinessError_1.BusinessError('APPOINTMENT_NOT_YET_DUE', 'O agendamento ainda não ultrapassou o período de tolerância de 15 minutos para ser classificado como No-Show.', 422);
        }
